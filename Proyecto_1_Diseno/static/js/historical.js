...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('searchBtn').addEventListener('click', () => UI.setVisible('#searchModal', true));
    document.getElementById('closeSearchModal').addEventListener('click', () => UI.setVisible('#searchModal', false));
    
    document.getElementById('btnHoy').addEventListener('click', establecerRangoHoy);
    document.getElementById('btn7Dias').addEventListener('click', establecerRangoUltimos7Dias);
    document.getElementById('btnBuscar').addEventListener('click', verHistoricoRango);
    document.getElementById('btnLimpiarMapa').addEventListener('click', limpiarMapa);

    // Eventos de controles del mapa
    document.getElementById('btnToggleMarcadores').addEventListener('click', toggleMarcadores);
    document.getElementById('btnAjustarVista').addEventListener('click', Map.fitBounds);
    document.getElementById('btnLimpiarGeocerca').addEventListener('click', Map.clearGeofence);
...
async function verHistoricoRango() {
    ...
    const data = await API.fetchHistoricalRange(params.inicio, params.fin, params.hora_inicio, params.hora_fin);
...
async function generarRutaOSRM(latLngs) {
    ...
    for (let i = 0; i < latLngs.length; i += CHUNK_SIZE) {
        ...
        UI.updateProgress('#routeProgressBar', '#routeProgressText', i, latLngs.length);
        
        try {
            const routeData = await API.fetchOSRMRoute(chunk);
            if (routeData && routeData.routes && routeData.routes.length > 0) {
                allRouteGeometry.coordinates.push(...routeData.routes[0].geometry.coordinates);
            }
        } catch (error) {
...
function toggleMarcadores() {
    ...
    UI.updateText('#btnToggleMarcadores', areMarkersVisible ? 'Ocultar Marcadores' : 'Mostrar Marcadores');
}
...
function actualizarInfoRecorrido(data) {
    ...
    UI.updateField('puntoInicial', `${inicio.lat.toFixed(5)}, ${inicio.lon.toFixed(5)} (${inicio.timestamp})`);
    UI.updateField('puntoFinal', `${fin.lat.toFixed(5)}, ${fin.lon.toFixed(5)} (${fin.timestamp})`);
...
function exportarDatos() {
    ...
    link.setAttribute("download", `historico_${UI.getValue('#fechaInicio')}_a_${UI.getValue('#fechaFin')}.csv`);
