...
async function generarRutaPorCallesRealtime(puntosRaw) {
  ...
  if (rutaOSRM && rutaOSRM.length > 0) {
    console.log(
      `✓ Segmento ${ultimoIndice} generado por calles (${rutaOSRM.length} puntos)`
    );
...
function actualizarPolyline() {
  if (trayectoria.length > 1) {
    ...
    if (trayectoriaVisible) {
      ...
      polyline.bindPopup(
        `Trayectoria: ${trayectoriaRaw.length} puntos GPS (${trayectoria.length} puntos en ruta)`
      );
    }
  }
}
...
export async function regenerarRuta() {
  ...
  const confirmar = confirm(
    `¿Regenerar toda la ruta usando ${trayectoriaRaw.length} puntos GPS?`
  );
  if (!confirmar) return;
